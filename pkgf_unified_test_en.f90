program pkgf_unified_test_en
        ...
        do b_idx = 1, size(story_hierarchy(s_idx)%bunsetsus)
            do t_idx = 1, size(story_hierarchy(s_idx)%bunsetsus(b_idx)%tokens)
                associate (token => story_hierarchy(s_idx)%bunsetsus(b_idx)%tokens(t_idx))
                    ...
                    write(*, '(A, " | ", F6.2, " | ", F10.5, " | ", ES10.2, " | ", F10.5)') &
                        token%text(1:24), & ! Approx width matching Python's <24
                        t, norm_v, div_val, det_K
    ...
    print *, "1. English linguistic flow analyzed with purified 12D geometry."
    print *, "2. Incoherence and awakening detected as physical spikes."
...
contains
    ...
    subroutine init_story_data()
        ...
        story_hierarchy(1)%bunsetsus(1)%tokens(1) = TokenData("This Agenda is", "Subject", 1.0_real64, 14)
        allocate(story_hierarchy(1)%bunsetsus(2)%tokens(1))
        story_hierarchy(1)%bunsetsus(2)%tokens(1) = TokenData("a plan of action.", "Entity", 1.0_real64, 17)
        
        allocate(story_hierarchy(2)%bunsetsus(2))
        allocate(story_hierarchy(2)%bunsetsus(1)%tokens(2))
        story_hierarchy(2)%bunsetsus(1)%tokens(1) = TokenData("The gears of the void,", "Subject", 5.0_real64, 22)
        story_hierarchy(2)%bunsetsus(1)%tokens(2) = TokenData("melon bread,", "Entity", 3.0_real64, 12)
        allocate(story_hierarchy(2)%bunsetsus(2)%tokens(2))
        story_hierarchy(2)%bunsetsus(2)%tokens(1) = TokenData("run backward", "Action", 8.0_real64, 12)
        story_hierarchy(2)%bunsetsus(2)%tokens(2) = TokenData("against gravity.", "Entity", 5.0_real64, 16)
        
        allocate(story_hierarchy(3)%bunsetsus(2))
        allocate(story_hierarchy(3)%bunsetsus(1)%tokens(1))
        story_hierarchy(3)%bunsetsus(1)%tokens(1) = TokenData("Suddenly, I wake up!", "Action", 8.0_real64, 20)
        allocate(story_hierarchy(3)%bunsetsus(2)%tokens(2))
        story_hierarchy(3)%bunsetsus(2)%tokens(1) = TokenData("Reflecting deeply", "Action", 6.0_real64, 17)
        story_hierarchy(3)%bunsetsus(2)%tokens(2) = TokenData("on my incompetence.", "Subject", 5.0_real64, 19)
    ...
    subroutine get_target_vector(token, vec)
        ...
        if (index(token%text, "wake up") > 0) then
            vec = vec * (-1.5_real64)
        end if
...
end program pkgf_unified_test_en
