...
def project_divergence_free(v, x, K):
    ...
    def calculate_local_div_g(current_v, current_x):
        ...
        for i in range(DIM):
            ...
            term_p, term_m = sg_p * flux_p[i], sg_m * flux_p[i]
...
story_hierarchy = [
    [ [("This Agenda is", "Subject", 1.0)], [("a plan of action.", "Entity", 1.0)] ],
    [ [("The gears of the void,", "Subject", 5.0), ("melon bread,", "Entity", 3.0)], [("run backward", "Action", 8.0), ("against gravity.", "Entity", 5.0)] ],
    [ [("Suddenly, I wake up!", "Action", 8.0)], [("Reflecting deeply", "Action", 6.0), ("on my incompetence.", "Subject", 5.0)] ]
]
...
def get_target_vector(token, role, intensity):
    ...
    # Awakening logic (Inversion of phase)
    if "wake up" in token:
        vec = [v * -1.5 for v in vec]
    return vec
...
def run_strict_analysis():
    ...
    x = [0.1] * 12
    t, dt = 0.0, 0.2
    print(f"{'Token':<24} | {'t':<6} | {'||v||':<10} | {'div_g(KX)':<10} | {'det(K)':<10}")
    print("-" * 75)
    for sentence in story_hierarchy:
        for bunsetsu in sentence:
            for token, role, intensity in bunsetsu:
                ...
                det_H, _ = renormalization_flow_invariants(H)
                det_K = calculate_det_12x12(K)
                norm_v = math.sqrt(sum(vi**2 for vi in v_proj))
                print(f"{token:<24} | {t:6.2f} | {norm_v:10.5f} | {div_val:10.2e} | {det_K:10.5f}")
        print("-" * 75)
    print("\n[Scientific Findings]")
...
if __name__ == "__main__":
    log_filename = "python_pkgf_12d_en_log.txt"
    class Tee(object):
    ...
    with open(log_filename, "w", encoding="utf-8") as f:
        ...
        try: run_strict_analysis()
        finally: sys.stdout = original_stdout
    print(f"\n[System] Results saved to {log_filename}")
